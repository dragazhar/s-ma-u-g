...
public class MealyFST extends FSM {

    public MealyFST(Alphabet alpha, Alphabet beta) {
	super();
    ...
    public void printToFile(String fileName, Behaviour behaviour)
	    throws IOException {
	...
	try {
		...
		int hd = StringDistances.apply(out1s, out2s,
			StringDistances.HAMMING_DISTANCE);
		res.println("<td>" + (out1s.length() - hd) + "</td>");
		res.println("</tr>");
